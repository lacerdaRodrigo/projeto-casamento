...
- **D16 — Redesign do painel** *(→ Seções 2/7):* interface refeita a partir do handoff de design. **Tema escuro (Nocturne) como padrão + botão de alternar pro claro** (rosé), persistido em `localStorage` sem flash. Navegação da árvore por **abas de tema + visão geral** (troca instantânea no cliente, sem refetch) em vez da lista corrida — reduz rolagem. **Hero** (contagem regressiva + anel de progresso), **barra empilhada** de orçamento (pago/comprometido/estimado + marca do teto) e **comparação estimado × real** por item. Nova tela **"Montar a árvore"** (`/montar`). **Feedback in-place:** salvar **não navega** — a página atualiza no lugar, com botão "Salvando…" e **toast client** de sucesso/erro; o editor do item **fecha ao salvar**. As regras de negócio (C1–C4) seguem intactas — o redesign é só apresentação/navegação.
- **D17 — Item direto no tema (subtema opcional)** *(→ Seções 1/5):* na prática o casal pensa em **2 níveis** (Tema → coisa com preço), não 3. Agora dá pra adicionar **itens com preço direto no tema**; o **subtema vira opcional** (só agrupamento). **Tema e subtema são grupos puros** — só o **item** carrega preço/status/prazo. Implementação **sem migração**: os itens "soltos" moram num subtema oculto de **nome reservado** (`(itens do tema)`), renderizado sem cabeçalho; a validação impede o usuário de colidir com esse nome. O botão "+ Subtema" está **escondido atrás de uma flag** (`MOSTRAR_ADD_SUBTEMA`, em `src/app/ui-flags.ts`) enquanto se testa sem ele — reversível num toque. A profundidade de 3 níveis do schema (D01) segue intacta.
- **D18 — Login e operação (app privado)** *(→ Seção 9):* para um casal, decidiu-se **e-mail + senha com conta fixa compartilhada** (`npm run seed:conta` cria/garante a conta a partir do `.env.local`) + **cadastro público fechado** no Supabase. **Confirmação de e-mail, SMTP, magic link e login Google foram dispensados** (complexidade sem retorno pro caso privado) — código do fluxo de confirmação fica dormente. **M3 (contas separadas + convite + papéis) não é necessário** enquanto a conta é compartilhada; a porta (RLS por membro) continua aberta. **Entrega concluída:** repositório no GitHub, **CI** (`ci.yml`: typecheck+testes+build), **deploy automático na Vercel** a cada push na `main`, **versionamento automático** (D15 via `semantic-release`, começando em `1.0.20`). RLS: auditoria das policies + smoke `test:rls` (anônimo bloqueado). **E2E** (Playwright) da fatia vertical.

---

*PRD **v1.3**. Estado: **V1 no ar** — M1+M2 concluídos, M4 (acabamento: redesign, filtros, template) feito; entrega (CI+deploy+versão) concluída (D18). **M3 (colaboração) dispensado** por ora (conta compartilhada — D18). Opcionais restantes: isolamento RLS entre 2 casórios provado por teste, updates otimistas, reordenar itens.*
